...
import me.james.upload.utils.PathUtil;
...
public class DeviceSchedulerTask extends TimerTask {
    ...
    public void run() {
        ...
        if (roots.size() > Objects.requireNonNull(new File(PathUtil.getLinuxMediaPath()).listFiles()).length) {
            // Some devices has been removed
            log.info("Some devices have been removed.");
            updateRoots();
        } else if (Stream.of(Objects.requireNonNull(new File(PathUtil.getLinuxMediaPath()).listFiles()))
                .map(File::getAbsolutePath)
                .anyMatch(c -> !roots.contains(c))) {
            // New devices loaded
            log.info("New devices detected.");
            try {
                Stream.of(Objects.requireNonNull(new File(PathUtil.getLinuxMediaPath()).listFiles()))
